...
// This loop checks ALL messages
  for(p=NewMail.begin(); p!=NewMail.end(); p++)
      ...
      if (rMsg.m_sKey == "UCTD_MSMNT_REPORT" && in_survey)
	{
	  ...
	  m_msmnt_report_que.push_back(value.c_str());
	  anneal.addMeas(buf);
	  num_meas += 1;
	  MOOSTrace("New measurement added, Total = %d\n", num_meas);
	}

       if (rMsg.m_sKey == "VEHICLE_REPORT" && in_survey)
	{
	  value = rMsg.m_sVal;
	  CMeasurement buf;
	  buf = anneal.parseMeas(value);
      ...
      else if (rMsg.m_sKey == "SURVEY_UNDERWAY")
	{
      ...
      else if (rMsg.m_sKey == "REPORT_REQ")
	{ 
	  vector<double> result;
	  anneal.getEstimate(result);
	  offset =     result[0];
	  ...
	  Notify("ANGLE", angle);	
       ...
       else if (rMsg.m_sKey == "NODE_REPORT")
      {
	m_comms=true;
...
void CFrontEstimate::SendVehicleReport()
{
      string str;
      unsigned int jj = m_msmnt_report_que.size();
	...
	str="empty";
      ...
      { 
       str = str + m_msmnt_report_que[0];
       m_msmnt_report_que.erase(m_msmnt_report_que.begin());
